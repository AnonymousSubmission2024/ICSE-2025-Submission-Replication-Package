...
static void MoveAllNodeInputEdges(Graph& graph, Node& src_node, Node& target_node) {
  ...
  auto input_edges = GetNodeInputEdges(target_node); 
...
/*
The following method has a failing test
Input the following graph:
...
Methods GetNodeInputEdges, AddEdge, RemoveGraphEdges function as expected
 */
...
static void MoveAllNodeOutputs(Graph& graph, Node& src_node, Node& target_node) {
  ...
  auto target_idx = target_node.Index();
  auto output_edges = GetNodeOutputEdges(target_node);
